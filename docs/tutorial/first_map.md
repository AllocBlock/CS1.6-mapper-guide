...
![](../videos/create_solid.mp4 ':include :type=video controls preload="metadata" width=60%')
- **3D视图移动视角**：按住```空格```，鼠标移到3D视图上，按住左键，此时可以按WASD来移动镜头了
	- 或者使用工具栏的“摄像机工具”![](../images/hammer_tool_camera.png)，就无需按空格了

![](../videos/move_camera.mp4 ':include :type=video controls preload="metadata" width=60%')
- **操控2D视图**：按住```空格```，然后按住鼠标拖拽来平移视图；滚轮可以放大、缩小视图。

![](../videos/change_view_2d.mp4 ':include :type=video controls preload="metadata" width=60%')
- **选择物体**：在工具栏点击“选择工具“![](../images/hammer_tool_select.png)，框选来选择刚刚建好的固体，或者点击中心来选择，选中的物体在2D视图里固体变成红框，3D视图里也会变红。

![](../videos/selection.mp4 ':include :type=video controls preload="metadata" width=60%')
?> 框选不能选择？默认设置下的选择可能需要按```回车键```确认，可以在设置里调整，勾选“选择对象不需要回车确定”；另外默认不需要点中心点也能选择物体，但是容易误选，所以一般勾上“仅点击中心点才能选择对象”。![](../images/hammer_setting_select.png ':size=40%')
...
![](../videos/edit_mode.mp4 ':include :type=video controls preload="metadata" width=60%')

- **更换纹理**：点击右上角纹理栏的浏览，选择一个合适的纹理作为当前纹理。然后选择物体，点击左侧工具栏的“应用当前纹理”，可以把纹理应用到选择的物体上。

![](../videos/change_texture.mp4 ':include :type=video controls preload="metadata" width=60%')
- **挖空固体**：现在这个固体还是实心的，hammer提供的功能可以挖空它，让它变成空心的。选中物体，然后点击 菜单栏→工具→挖空，默认值32即可（表示挖空后墙面厚度是32）。

![](../videos/hollow_solid.mp4 ':include :type=video controls preload="metadata" width=60%')
...
![](../videos/create_point_entity.mp4 ':include :type=video controls preload="metadata" width=60%')
- **修改实体类型为CT出生点**：在点实体上右键（只能在2D视图），选择属性，会弹出实体属性窗口。下拉菜单里选择```info_player_start```，它对应了CT出生点。

![](../videos/set_point_entiy_type_ct_start.mp4 ':include :type=video controls preload="metadata" width=60%')

- **T出生点**：同样的方法，我们制作一个T出生点，T出生点对应的实体叫做```info_player_deathmatch```，把他们放在两侧，可以得到这样的结果。
	- ![](../images/box_with_starts.png ':size=50%')
- **制作灯**：没有灯的地图是漆黑一片的，我们放一个灯来照明。灯也是通过点实体来实现的，对应的名称叫做```light```，制作一个灯放在天花板上，调整颜色和亮度。
	- 如视频中放置了一个灯在天花板附近，颜色淡橘色，亮度为300。

![](../videos/add_light.mp4 ':include :type=video controls preload="metadata" width=60%')
...
- **编译选项**：点击快捷工具栏最右侧的“编译地图”![](../images/hammer_shortcut_compile.png)，会弹出设置按钮，几个设置分别如下：
	- ![](../images/hammer_compile_setting.png ':size=40%')
...
![](../videos/play_first_map.mp4 ':include :type=video controls preload="metadata" width=60%')
